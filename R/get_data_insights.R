...
#' @description Get detailed insights about the data like number of rows,
#' columns and some basic statistics such as mean
...
getDataInsight <- function(temp) {
  ...
  # WARNING: This variable is not used anywhere.
  df <- data.frame(Column = "Char", Type = "Char", missing = "G")

  y <- do.call(rbind, lapply(names(temp), FUN = function(x) {

    mean <- ifelse(is.numeric(temp[[x]]), round(mean(temp[[x]], na.rm = TRUE),
                                                digits = 2), NA)

    median <- ifelse(is.numeric(temp[[x]]), round(stats::median(temp[[x]],
                                                                na.rm = TRUE),
                                                  digits = 2), NA)

    sd <- ifelse(is.numeric(temp[[x]]), round(stats::sd(temp[[x]],
                                                        na.rm = TRUE),
                                              digits = 2), NA)

    variance <- ifelse(is.numeric(temp[[x]]), round(stats::var(temp[[x]],
                                                               na.rm = TRUE),
                                                    digits = 2), NA)
  ...
  if (length(getnumericCols(temp)) == 0) {
    ...
    cor_mat <- as.data.frame(stats::cor(temp[, getnumericCols(temp)] %>%
                                          na.omit()))
  }

  return(list(Rows = number_of_rows, Columns = number_of_columns, Types = y,
              cor_matrix = cor_mat))
